...
%0 theta change
    basemprimendpts0_c(1,:)  =  [  1   0   0   forwardcostmult  0];
    basemprimendpts0_c(2,:)  =  [  8   0   0   forwardcostmult  0];
    basemprimendpts0_c(3,:)  =  [ -1   0   0   backwardcostmult 0];    
%1/16 theta change
    basemprimendpts0_c(4,:)  =  [ 28  5    2   forwardandturncostmult  max];
    basemprimendpts0_c(5,:)  =  [ 28 -5   -2   forwardandturncostmult -max ];
%turn in placeturninplacecostmult
    basemprimendpts0_c(6,:)  =  [ 16  1   1   forwardandturncostmult   max/2];
    basemprimendpts0_c(7,:)  =  [ 16 -1  -1   forwardandturncostmult  -max/2];
...
%1/16 theta change going backward
    %basemprimendpts0_c(8,:)  =  [10  1   2   forwardandturncostmult  max];
    %basemprimendpts0_c(9,:)  =  [10 -1  -2   forwardandturncostmult  -max];
%forward diagonal
...
%1/16 theta change
    basemprimendpts26p6_c(4,:)  =  [ 12   7   2   forwardandturncostmult  max];
    basemprimendpts26p6_c(5,:)  =  [ 12   4   -2   forwardandturncostmult -max];
%turn in place
...
%1/16 theta change
    basemprimendpts67p5_c(4,:)  =  [ 7   12   -2   forwardandturncostmult  -max];
    basemprimendpts67p5_c(5,:)  =  [ 4   12    2   forwardandturncostmult max];
%turn in place
...
fout = fopen('teste7mprim_5cm.mprim', 'w');
%write the header
    ...
    for primind = 1:numberofprimsperangle
        ...
        if ( baseendpose_c(3) == 0) % move forward            
            ...
            for iind = 1:numofsamples
                ...
                for iind = 1:numofsamples     
                     dsteer = steer/(numofsamples -1);
                    dt = (iind-1)/(numofsamples-1);
                           
                    
                        intermcells_m(iind,1) =  startpt(1) + dt*tv*cos(startpt(3) + dt*(tv/L)*tan((iind-1)*dsteer ));
                        intermcells_m(iind,2) =  startpt(2) + dt*tv*sin(startpt(3) + dt*(tv/L)*tan((iind-1)*dsteer ));
                        intermcells_m(iind,3) =  startpt(3) + dt*(tv/L)*tan((iind-1)*dsteer );
